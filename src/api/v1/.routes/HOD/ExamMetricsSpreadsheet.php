  ...
  private function generateStudentSheet($subject)
  {
    ...
    foreach ($subject->students as $s) {
      ...
      $row = [
        ...
        '',
        '',
        '=RANK(I'. $i . ',I$4:$I$200, 1)',
        '=ROUND((Q' . $i . '*Q$1+T' . $i . '*T$1+X' . $i . '*X$1+Y' . $i . '*Y$1+W' . $i . '*W$1+AA' . $i . '*AA$1+AC' . $i . '*AC$1+AE' . $i . '*AE$1+AG' . $i . '*AG$1)/COUNTA(M' . $i . ',O' . $i . ',Q' . $i . ',T' . $i . ',W' . $i . ',Y' . $i . ',AA' . $i . ',AC' . $i . ',AE' . $i . ',AG' . $i . '),2)',
        $s->mlo0 ?? $s->mlo1 ?? '',
        $s->mlo1 ?? '',
        '',
        '',
